...
def expand_file_inputs(
    ...
    # Use fnmatch for proper glob pattern matching (more efficient and correct)
    import fnmatch

    filtered_files = []
    for file in all_files:
        if not any(fnmatch.fnmatch(file, f"*{pattern}*") for pattern in ignore_patterns):
